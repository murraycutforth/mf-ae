...
def main():
    ...
    num_spheres_per_vol_nominal = np.random.randint(1, 40, NUM_SAMPLES)

    for i in tqdm(range(NUM_SAMPLES)):
        n_spheres_nominal = num_spheres_per_vol_nominal[i]
        centers = np.random.rand(n_spheres_nominal, 3)

        # Radii are log-normal distributed
        radii = np.random.lognormal(mean=1.0, sigma=0.5, size=n_spheres_nominal)  # Mean e^1=2.7
        dx = 1.0 / VOL_SIZE
        radii *= dx

        # Only keep non-overlapping spheres
        n_spheres = 0
        ...
        for center, radius in zip(centers, radii):
            if all(np.linalg.norm(center - np.array(valid_center)) > (radius + valid_radius) for
                   valid_center, valid_radius in zip(valid_centers, valid_radii)):
        ...
        sdf_exact = multi_sphere_sdf(centers, radii, VOL_SIZE)
        ...
        for interface_type, epsilon, outdir in interface_type_generator(interface_types, epsilons):
            ...
            np.savez_compressed(outdir / f'spheres_{i}.npz', phi=phi, n_spheres=n_spheres, radius=radii)
